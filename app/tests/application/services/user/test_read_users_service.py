...
from app.application.services.user.read_users_service import GetUsersServices
...
def service(user_repository_mock):
    return GetUsersServices(user_repository=user_repository_mock)


def make_user(username="gabriel", email="g@email.com", role="USER"):
...
def test_should_raise_forbidden_error_when_user_has_no_permission(
    ...
    with patch(
        ...
        side_effect=ForbiddenError("Você não tem permissão para realizar esta ação."),
    ):
...
def test_should_return_users_dto_when_admin_requests_users(
    ...
    users = [
        make_user("gabriel", "g@email.com", "admin"),
        make_user("ana", "ana@email.com", "user"),
    ]
